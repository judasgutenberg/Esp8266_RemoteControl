...
void runCommandsFromNonJson(char * nonJsonLine, bool deferred){
  ...
  if(commandId) {
    ...
    if (command == "reboot slave") {
    ...
    } else if (command.startsWith("dump slave eeprom")) {
      loadAllConfigFromEEPROM(1);
      ...
      int bytesUsed = loadAllConfigFromEEPROM(2);
      textOut("Slave EEPROM bytes used: " + (String)bytesUsed + "\n");
...
void setup(){
  ...
  delay(10);
  initConfig();
  if(loadAllConfigFromEEPROM(0) != 1) {
    if(ci[DEBUG] > 0) {
