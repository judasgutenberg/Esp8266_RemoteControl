...
removes the quotes before passing the arguments to the command handler.  Configuration commands can refer to configuration items by name or number if sent from via http but if sent via socket or serial, they have to refer to the items by number.  
