...
## Database Design
I have tried to conform to a consistent naming convention in the database. I use snake_case and end solitary primary keys with "_id."  Compound primary keys are only used on a handful of tables, and the makeshift framework I am using to present web-based administrative tools generally does not expect compound primary keys (and so cannot edit tables with them).  That said, there are a few tables where tenant_id is part of a compound primary key.  This includes the table <b>sensor</b>, and I have made the framework capable of editing records in such tables.  The framework also happily provides tools to add records to tables with compound primary keys that provide mapping functions (such as device_sensor).
