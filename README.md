...
Every tenant gets its own set of credentials for APIs it has access to, which are added via the API Credentials link in the tenant list.  Some APIs can actually act like devices,  even logging data to the device_log table. These are called "pseudo devices" and their "device_id" (which is called "pseudo_device_id") is set with the API credentials.  Then that API is polled automatically by the backend to retrieve data so it can be stored in the device_log table.  This is how I track my dog Charlotte when she is wearing her proprietary Tractive tracker, since that is not a device I control; I can only get data via its cloud-based API.
