...
Devices are configured various ways, though the simplest way is via alterations on the device itself to array items in config.cpp. There are four arrays whose members are exposed in there, ci for configuration integer, cs for configuration string, cis for configuration integers for an attached slave, and css for parsing configurations on an attached slave.  All these configuration items can be altered remotely via the Instant Command tool (see **Commands**).  Simply issue a <b>set [the item's name, such as fram_index_size] [value]</b>.  You can see the existing value by issuing a <b>get [lowercase version of the item's name, such as fram_index_size]</b>.  If you have an I2C slave attached running the latest version of the firmware, these configuration values can be persisted in the EEPROM on the slave by issuing a <b>save entire config</b>. After that, the EEPROM version will be the configuration used by the device, so long as ci[SLAVE_I2C] doesn't change (though you can change it to communicate with other slaves).   Obviously, there is a lot of power in this capability and it is easy to put your remote device in an unresponsive state if you send the wrong data, such as the name of the server it is supposed to be communicating with.  
