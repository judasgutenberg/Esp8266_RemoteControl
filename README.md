...
https://github.com/judasgutenberg/Generic_Arduino_I2C_Slave and just add the I2C address of the slave Arduino to the device_type_feature record. Arduinos based on Atmega328s and Atmega168s are ideal for use as slaves.
...
This code uses a mix of server-side and client-side rendering of display elements, depending on which method is appropriate. Some components (such as the graphs output for weather and inverter data) use modern single-page techniques in vanilla Javascript, though in many places it makes more sense to render HTML in the backend and to provide conventional navigation between the pages.  Often the HTML in such pages is later modified by Javascript. When you aren't working in someone else's framework, you can make things behave in precisely nuanced ways without having to build your own framework on top of it (anyone who has tried to implement unusual systems in a framework knows what I mean).

Another important philosophy is that I want to support older hardware with my code, especially in the web client.  Not only do I have a fondness for old Espressif microcontrollers, but I love old Chromebooks, the kind that kids might've been using in school well before the pandemic. I have a half dozen of them in various places, and they typically cost $40.  They have all the advantages of a tablet, yet they also have a physical keyboard. And they're disposable enough for me to use in the tub (where I've never yet lost one). But I hate it when a website (yes you, Reddit!) tells me that my Chromebook is no longer supported, especially given how easy it is to provide support.  (In my code, the only change I've had to make was to stop using the Javascript replaceChildren() method.)  If you're insisting on using all the latest Javascript methods and constantly throwing up alerts telling your users to "upgrade," what you're really saying is that landfills don't contain quite enough eWaste. Solark's dashboard was one of the offenders, and it's great to have completely replaced that with the energy tab in this application.

I prefer my code to be as straightforward as possible. I don't, for example, appreciate clever one-line solutions to programming challenges and prefer to use explicit loops where it is obvious to as many viewers as possible what precisely is going on.  Similarly, I prefer an imperative over a declarative coding style, and I've never been enamored with an object-oriented approach, particularly for cases where it just gets in the way.  This is not to say this code contains none of those things, but it's not a showcase for any particular coding paradigm.
...
One final thing I'd like to mention regarding my philosophy concerns apps, the things that you find yourself constantly installing on your smart phone. I hate them. They frequently have names and icons unrelated to their function, and it's unlikely you use any one app very often. But when you do go to use one, you've forgotten what app it was that you need to do the rare thing you do. For example, back before I realized how terrible apps were, I made the mistake of buying a smart bulb at Home Depot.  To control that smart bulb, I had to install an app named "Wiz" on my phone.  But I almost never turn that lightbulb on, because I'm not the sort of person who keeps my phone close by me at all times.  Meanwhile, that lightbulb cannot remember its state through a power outage, which is something that happens once ever two months or so where I live. So after the power comes back on after such an outage, that stupid smart bulb is shining brightly, and my wife wants to know how to turn it off.  The only way to do that is with that app on my phone, but it's been two months and I've forgotten what the name of the app is.  A much more open approach to controlling devices like that is to provide an interface on a web page.  Then I can use any web browser to control it and I can remember where to go by either storing a bookmark or saving a url in a text file.  Not only that, but any device controllable by a web interface has an API that can be transplanted elsewhere, perhaps to be placed on a custom dashboard, possibly a web page where you could control all your devices in one place instead of across an archipelago of shouty apps constantly nagging to be updated.  This is why I have not made an app for the system described here.
