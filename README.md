...
## Off-grid vs On-grid
My use-case for this system is an off-grid Adirondack cabin that nevertheless has internet access via the cellphone network (just barely; I use a Moxee cellular hotspot placed in the focus of a carefully-directed 24 dB parabolic dish). If your setup has no way to connect to the ihternet, you could benefit from the data logging an automation capabilities on this system by just hosting the server component locally on something like a Rasperry Pi Zero, which could then collect and log data from numerous nearby ESP8266s that could be controlling solar energy resource allocation.
