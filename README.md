...
## Remote Control
The model of remote control this system uses is server-as-truth. Every time a device polls the server, it gets the server's idea of what the state of its features should be.  The device then dutifully changes the device features as necessary.  The only exception to this behavior is cases where a Local Remote makes a change to a device_feature on the device directly, and this sets a flag telling the device (and the server) that data from device should flow in the opposite direction, from device to server. In that case server-as-truth is overriden and the server updates its state accordingly.
