...
## Overview
If you need complete control of all the code of a robust multi-user/multi-tenant remote-control, automation and weather data logging system without infuriating compilation and dependency headaches (in Javascript or PHP), you might be interested in this repository. This is a system of distributed microcontroller-based devices that check in regularly with a server that <em>you completely control</em> to both log data and pick up instructions for what to do, and the code for all of the software layers is in this repository. I developed it to more efficiently use solar energy at my off-grid Adirondack cabin while also keeping the internet accessible and the pipes from freezing without entrusting some rando with my data, and no existing software did what I needed.  This started out as a simple multi-probe weather monitoring system (<a href=https://github.com/judasgutenberg/ESP8266-Micro-Weather target=esp>ESP8266 MicroWeather</a>) to which I first added reliability automation (<a href=https://github.com/judasgutenberg/Hotspot-Watchdog  target=hot>Hotspot-Watchdog</a>), then remote control, then solar inverter monitoring, then cloud-based automation, a multi-tenant security model (so others could use the same server installation as a backend), OTA updates, and decision-making from weather forecasts. It even features a live command-line query and reconfiguration system with support for a number of Unix commands that is similar to the Python REPL and is can be reached via serial or web interface at two different levels of latency.

For monitoring multiple weather sensors (placed, say, in various places inside and outside a building), there is a multi-plot graph displaying data across a number of time scales.
...
A web-based tool (tool.php) allows you to edit the value column of a table called device_feature, where device_id is equal to ci[DEVICE_ID] specified in config.cpp in the Arduino-compiled code (the files in firmware/esp8266) uploaded to your ESP8266 device.  Once installed on the ESP8266, it regularly polls the server (make sure to set host_get in config.cpp before compiling) to look for information in those device_feature records, and it automatically copies data from the value column to the pins they're connected to (which is specified in device_type_feature).  These pins can then be used to turn on relays, which allow you to control potentially large loads in remote locations. I'm using this system to remotely turn on the boiler, hot water heater, a 240 volt EV outlet, a 240 volt dual-zone minisplit, and three different 120 volt outlets connected to electric space heaters in my off-grid cabin.  Obviously, this involves working with dangerous voltage, so if you don't know anything about electrical wiring and basic safety with such things, you should instead use this to control low-voltage gizmos like battery-powered radios, toy motors, or LEDs.

Device_features can also be sensors reachable by a GPIO pin or I2C, and if multiple sensors are attached to an ESP8266, the additional ones will be identified by device_feature_id if they were created in the backend or configured by a configuration string in CIS[SENSOR_CONFIG_STRING].

One important caveat: this system is one where the server tells the microcontroller what pins do what and it can change assignments without a need to reflash or to even restart the microcontroller. For some pins (notably GPIO10 and GPIO9 on an ESP8266, which are used for communicating with on-board flash storage), setting them to outputs and forcing them to take a value will cause the ESP8266 to crash and restart. So be sure to properly test all your pin control arrangements while local to the ESP8266 before using it remotely. 
...
Note that all the databases referred to are MySQL databases that exist entirely on a server somewhere on the internet (or locally in your local network, if you choose to work that way).  No databases are implemented on actual microcontrollers or in web browsers.
...
If you have a slave running the version with commands and <a href=https://github.com/judasgutenberg/twiboot/blob/master/README.md target=twiboot>my special customized version of Twiboot</a>, you can update its firmware <em>entirely over I2C</em> by issuing an <b>update slave firmware [location of slave firmware hex file]</b> to the master.

## Arduino Setup
Some of the files (the files in firmware/esp8266) are designed to be compiled in the Arduino environment and then uploaded to an ESP8266. (I used a NodeMCU, which is cheap, reasonably small, and has conveniently-gendered pins making it easy to work with.)  The Arduino code is designed to be able to handle a diverse collection of common sensors with the ability to dynamically change sensor types without requiring a recompilation or even a restart.  This requires, of course, that the libraries for these devices are all installed at compilation time, which has used up less flash than I'd initially expected. Currently, the device libraries are as follows:
<ul>
...
To get this installed, first create an initial database by running remote_control.sql on the server and then change config.php so the PHP will know how to connect to the database.  You then create a tenant and a user connected to that tenant via tenant_user and give that tenant a storage_password so that the ESP8266 will be able to authenticate communication with the backend (all this can be done under the tenant tab in the web UI if your user has 'super' status).  This storage password is then set as cs[STORAGE_PASSWORD] (along with other important config information like your WiFi credentials and the domain and path where your backend is hosted) in config.cpp before compiling the Arduino code and uploading it to an ESP8266.  Then you create a device and give it device_features, and set the device_id of the ESP8266 to the ci[DEVICE_ID] of that device in config.cpp so that the backend will know which ESP8266 is polling to log sensor data and perhaps pick up remote control settings and additional sensors.

Now you need to create a new account with the web front end. The first user in the database is automatically given the role of <b>super</b>, which can edit the user table, create reports, and administer other tenants in the database, among other privileges.  That user also automatically gets a tenant, connected to the user via the tenant_user table.  Superusers can manage the connection of tenants to users via the user or tenant editors.  Users who need tenants can either ask the superuser to connect them to their tenant or new users can be invited to join their tenant by using a special link (which can be produced by one of the utilities in the Utilities tab).  

From there, here's an overview of how to set up control for a particular system controlled by a pin on your ESP8266:

1. Add a record to the device table to represent your particular ESP8266.  In the code, device_id and location_id refer to the same entity. Your ESP8266 will send that id as device_id to the server whenever it polls it to get updates on what values the device with its pins should have.  You will need to set ci[DEVICE_ID] to this value in config.cpp before compiling the code for you ESP8266.
...
## Users and Tenants
To login, every user must have credentials in the user table.  Users belong to a tenant, while devices also belong to the tenant.  Users typically can access all the devices belonging to their tenant but cannot access devices belonging to tenants they do not belong to.   Most other tables relate to tenants so that every tenant can have its own unique configuration. When new tenants are first created, they are given a set of related entities (such as weather_conditions, reports, and device_type_features) from a "template" tenant with a tenant_id of 0.  The exact values in this template can vary depending on who is curating it. Typically one tenant is a model tenant and is used as the basis for the template tenant.  There is a utility for copying certain records from any particular tenant to the template tenant. When this is done, the tenant being copied from is said to be the "model" tenant.  At any time any template can have their setup records overwritten with records from the template tenant using another utility, though this is typically only done when a tenant is first created.

## Device Configuration
Devices (that is, microcontrollers) are configured various ways, though the simplest way is via alterations on the device itself to array items in config.cpp prior to compilation. There are four arrays whose members are exposed in there, ci for configuration integer, cs for configuration string, cis for configuration integers for an attached slave (only one is supported initially, though these values can be changed on multiple slaves during runtime), and css for parsing configurations on an attached slave.  All these configuration items can be altered remotely via the Instant Command tool (see **Commands**).  Simply issue a <b>set [the item's name, such as fram_index_size] [value]</b>.  You can see the existing value by issuing a <b>get [lowercase version of the item's name, such as fram_index_size]</b>.  You have three options for persisting configuration that is at variance with values in config.cpp.  You can  save it in a file called config.cfg in the LittleFS volume the device's flash, you can save it in EEPROM on an attached I2C Slave, or you can save it in an attached FRAM.  (You configure which to use by setting ci[CONFIG_PERSIST_METHOD] in config.cpp). If you set that to 0, it will use config.cfg in the LittleFS volume. If you set it to 1 and have an I2C slave attached running the latest version of the firmware, these configuration values can be persisted in the EEPROM on the slave.  And if you set it to 2 and have a FRAM attached, it will save it there.   In any case, you save the existing configuration (possibly modified by configuration commands) by issuing a <b>save master config</b>. After that, the flash or EEPROM version will be the configuration used by the device, so long as information pointing at this configuration information (such as ci[CONFIG_PERSIST_METHOD]) or perhaps ci[SLAVE_I2C]  doesn't change.   Obviously, there is a lot of power in this capability and it is easy to put your remote device in an unresponsive state if you send the wrong data, such as the name of the server it is supposed to be communicating with.  

In addition to configuration items on the master controller, this system also allows a user to change and view configuration items on an I2C slave using the <b>set slave xxx</b> and <b>get slave xxx</b> commands.  Slaves also have commands that can be specified by number (these the numbers for the commands can be looked up in the file i2cslave.cpp), and to run them you issue a <b>run [command_number] [parameter1] [possible_parameter2]</b>.
...
The firmware in any ESP8266 in this system can be updated by copying the .bin file of its image (which is retrievable in the file system the Arduino IDE creates, though that isn't as straightforward as the folks at Arduino could've made it; why couldn't this be exposed in a menu?) to the folder on the web server whose name is given as $flash_directory in config.php.  Then you simply issue the command <b>update firmware <url></b> as an instant command.  If the file is a genuine image, the file will flash and the ESP8266 will reboot with the new firmware.  An alternative method that is even more reliable is to first download the firmware into the LittleFS file system (issue a <b>download <url></b> of a known good firmware url), ascertain that the image is the correct size, and then issue a <b>local update firmware <filename></b>. Note:  if this should fail because the file is for the wrong firmware, you may have to journey to the ESP8266 to fix it.

There is also the ability to remotely upload new firmware to an I2C slave entirely via I2c.  This allows slaves to be left in-circuit indefinitely and no longer require USB or other serial hardware unless needed for some specific purpose.  To implement this functionality, the slaves must have <a href=https://github.com/judasgutenberg/twiboot_for_arduino  target=boot>my modification of the twiboot I2C bootloader</a>.

## APIs and Pseudo Devices
Every tenant gets its own set of credentials for APIs it has access to, which are added via the API Credentials link in the tenant list.  Some APIs can actually act like devices,  even logging data to the device_log table. These are called "pseudo devices" and their "device_id" (which is called "pseudo_device_id") is set with the API credentials.  Then that API is polled automatically by the backend to retrieve data so it can be stored in the device_log table.  This is how I track my dog Charlotte when she is wearing her proprietary Tractive tracker, since that is not a device I control; I can only get data via its cloud-based API.  It's also how I began tracking my AmeriGas propane fuel levels after AmeriGas replaced my homemade magnetometer (clipped into the RemoteReady slot) with a fancy satellite-based data transmitter.

## Data Logging
The ESP8266 transmits delimited data in a querystring to the backend at a regular polling interval set in config.cpp (or via the command <b>set DATA_LOGGING_GRANULARITY xx</b>). This data includes temperature, air pressure, humidity, and other possible weather sensor values, as well as the states of GPIO-attached devices being controlled remotely.  It can also transmit positional data from a mobile ESP8266. All of this data is processed by server.php and logged in the device_log table.  If a FRAM or flash I2C memory is attached to the ESP8266 and properly configured in config.cpp, then logging will be stored locally when the internet cannot be reached and then gradually sent to the backend once the internet reappears.  Depending on the amount of data being stored locally per record, a $6 32K FRAM can hold about 1000 records.  An ESP8266 has no built-in real time clock, so proper timestamping requires access to the internet (at least initially) unless a real time clock is added via I2C and its address specified in config.cpp.   I use polling instead of other (more responsive) forms of communication (such as web sockets) to remain as compatible as possible with Apache while making it possible for data-logging-only devices to spend most of their time asleep.  (Though I do now support web sockets to facilitate real time command-based ineraction in cases where this is useful.)  
...
Each microcontroller regularly polls the backend, starting when it boots up.  Every time it polls the backend, it tells the backend what it knows about its state, particularly the values of its sensors, the state of any device_features it has, and the last command_id and command_log_id it has processed. Initially the microcontroller retrieves the information it needs to know more about itself. From then on, it is mostly receiving updates about the states of its device_features, which are usually the same from one polling to the next. Occasionally there will be a command found in the command_log that lacks a result_recorded timestamp, and this will be sent instead of the usual device_feature data.  The microcontroller will then perform the command and transmit any result back to the backend, which will either send any additional unprocessed commands or go back to sending device_feature updates.  Three commands, <b>reboot</b>, <b>update firmware</b> and <b>watchdog reboot</b>, cannot be processed immediately, as that would cause the microcontroller to reboot and fail to acknowledge receipt of the command. So reboot is deferred until just after its receipt is acknowledged.  (Potentially other commands could be deferred in this same way.)

## Serial Data Parsing
The firmware includes a parser that allows the ESP8266 to monitor a serial line (usually on the "swapped" RX pin, using serial.swap()) and parse out data known to be present in the serial data in predictable locations (offsets from known search values).  The parser was originally written to parse the hexdump format, though it can theoretically also parse ASCII dumps.  To configure the serial parser, place a file called serialparser.cfg into the on-device LittleFS file system (you can use the download function to do this if you know the URL of a good serialparser.cfg). The format of this file is as follows:  each line in the file configures a "block" which will look through a subsection of the serial data located between a beginning passage and an ending passage.  Within this block, the parser looks for addresses of a given sequence.  The parser then finds the 16-bit values between two offsets.  There can be as many as 12 pairs of such offsets one after the other.

```
<blockOneStartSequence><blockOneEndSequence><address1><startOffset0><endOffset0><startOffset1><endOffset1>...<startOffsetN><endOffsetN><address2>...
<blockTwoStartSequence><blockOneEndSequence><address1><startOffset0><endOffset0><startOffset1><endOffset1>...<startOffsetN><endOffsetN><address2>...
...
```
...
which would set the connected device_feature's value to the value of management_rule.result if the condition is met and allow_automatic_management in the device_feature record is true.  You can also use aggregate functions in a token if you include colons.  The term before the first colon is the aggregator function from MySQL, such as avg, count, max, min, std, sum, or variance.  You can also use "integral" as an aggregator function, since you can chose both the column being aggregated and a timescale column, and the token parser code will generate the appropriate sql to get a result.  If you choose an aggregator function, you have to end the token with another colon and a timespan such as today or yesterday. If you want to change the time column from `recorded` you can end the management_column with a slash and then include a time column. You can include PHP statements in the conditionals to refer to concepts such as time using PHP or any of the PHP functions used by the system that you have permission to run.  For example,   this management rule:

<code>

	
(&lt;inverter_log[].solar_power&gt; > 2000 && &lt;inverter_log[].battery_percentage&gt; &gt; 90) 
|| &lt;integral:device_weather_forecast_hour[16].forecast_solar_radiation/forecast_hour:today&gt; &gt; 7000  && hour()  &lt; 10
...
## Commands
This system includes a comprehensive interactive microcontroller command system for checking and changing configuration parameters, querying states, and triggering numerous behaviors. Commands can be sent to a microcontroller via three different transport methods:  serial, polling-based http, and immediate (WebSocket-based) internet.
...
the "values" parameter in the form JSON could also be a SQL string to generate a list of options from the database.  If so, the SQL needs to return a 'text' column for a proper dropdown list of options to be displayed. (Obviously, anyone permitted to write such reports could wreak havoc with malicious SQL placed here to drop tables, etc.  With great power comes at least some responsibility, amirite, Yahweh?) A log of each report that is run is kept, and each log item is accessible via the web UI with enough information to allow it to be re-run with the same set of parameters as the original run.

Here is the JSON for a report that can generate two different graphs ("good graph" and "better graph" will appear in the output format dropdown as options, in addition to the always-available options for HTML and CSV):
