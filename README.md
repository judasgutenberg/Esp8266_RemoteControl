...
But the main feature in this system is that it allows you to remotely control devices across the internet and also supports automation based on the values of sensors known to the central database.  It does this using a server running PHP/MySQL.  There are relatively few dependencies:  PHP, MySQL, APCu, cURL, and the Chart.js graphing library, so this is not much of a headache to get working on just about any server where you have a modest amount of control.  
