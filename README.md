...
This system is multi-user and supports multiple user accounts, each with potentially multiple devices.  First register your user by creating a new account with the web front end. Then use whatever database tool you have to change its user role to "admin," allowing you unfettered access.  From there, here's an overview of how to set up control for a particular system controlled by a pin on your ESP8266:
