...
## Commands
The ESP8266 is responsive to commands sent via the serial line (at this time, though, this does not seem to work when WiFi is active). These commands can do things like reboot the ESP8266, display how much memory is available, send infra-red commands, and dump stored records from the FRAM if there is one.  If these commands are sent via the Instant Command tool (reachable via the Utilities tab in the web app), then any responses from the command are sent back to that same window.  The command system, like the device_feature control system, has an inherent latency dictated by the polling rate of the ESP8266 (set in config.c, but also changeable via command), but it's fast enough for most debugging or control needs.
