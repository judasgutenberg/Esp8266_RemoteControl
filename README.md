...
## Future Features
As of late August, 2025, all of the essential (and many of the nice-to-have) features had been implemented. The one feature that I would like to build out is one where the .ino files used to generate ESP8266 (and perhaps other) firmware would be templateized so that they could be flexibly produced. (Currently all ESP8266s in this system receive the same firmware, with the only differences being configuration.) I eventually want to build an application to populate these templates, do the code compilation, and flash to the microcontrollers, completely bypassing the Arduino IDE.  This would give this system the core functionality of the ESPHome project, though it would have some advantages due to the database at its core and its use of GUIs.
