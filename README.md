...
Obviously, if one is insisting on making all their own database access and login methods, there is a strong possibility of security vulnerabilities creeping into the design. I'm aware of most of these, though it's possible I've overlooked something.  (It's open source, so feel free to make pull requests!)  My reporting system gives immense powers to super users, for example, but that's something I personally need and have gotten used to having. And it's easy enough not to give access to report writing (or even execution) to your users under this system.
