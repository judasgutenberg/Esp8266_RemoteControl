...
## Setup
This system is both multi-tenant and multi-user, and each tenant can have potentially multiple devices.  First register your user by creating a new account with the web front end. The first user in the database is automatically given the role of admin, which can edit the user table among other priviledges.  You will also need a tenant and an entry connecting the user and the tenant in the tenant_user table (for which there is currently no UI support).  From there, here's an overview of how to set up control for a particular system controlled by a pin on your ESP8266:
