...
Devices (that is, microcontrollers) are configured various ways, though the simplest way is via alterations on the device itself to array items in config.cpp. There are four arrays whose members are exposed in there, ci for configuration integer, cs for configuration string, cis for configuration integers for an attached slave, and css for parsing configurations on an attached slave.  All these configuration items can be altered remotely via the Instant Command tool (see **Commands**).  Simply issue a <b>set [the item's name, such as fram_index_size] [value]</b>.  You can see the existing value by issuing a <b>get [lowercase version of the item's name, such as fram_index_size]</b>.  You have two options for persisting configuration that is at variance with values in config.cpp.  You can either save it in a file called config.cfg in the LittleFS volume the device's flash, you can save it in EEPROM on an attached I2C Slave, or in an attached FRAM.  (You configure which to use by setting ci[CONFIG_PERSIST_METHOD] in config.cpp). If you set that to 0, it will use config.cfg in the LittleFS volume. If you set it to 1 and have an I2C slave attached running the latest version of the firmware, these configuration values can be persisted in the EEPROM on the slave.  In either case, you save the existing configuration (possibly modified by configuration commands) by issuing a <b>save entire config</b>. After that, the flash or EEPROM version will be the configuration used by the device, so long as information pointing at this configuration information (such as ci[CONFIG_PERSIST_METHOD]) or perhaps ci[SLAVE_I2C]  doesn't change.   Obviously, there is a lot of power in this capability and it is easy to put your remote device in an unresponsive state if you send the wrong data, such as the name of the server it is supposed to be communicating with.  
