...
//also returns pk by reference, and it is an array
function schemaArrayFromSchema($table, &$pk) {
  ...
  if ($result) {
    ...
    foreach ($rows as $row) {
      ...
      if ($row["Key"] == "PRI") {
        $pk[count($pk)] = $fieldName;
        // Only hide if the column is AUTO_INCREMENT
      ...
      if ($fieldName != "tenant_id") {
        $record = ["label" => $fieldName, "name" => $fieldName, "type" => $type, "autogenerated"=> true, "extra"=>gvfa("Extra", $row)];
...
function genericEntityList($tenantId, $table, $outputFormat = "html") {
  ...
  if($thisDataResult) {
    ...
    if(strtolower($outputFormat) == "csv") {
      ...
      $out .= genericTable($thisDataRows, $headerData, $toolsTemplate, null, $table, $pk[0]);
    }
  }
  return $out;
}
...
function genericForm($data, $submitLabel, $waitingMesasage = "Saving...", $user = null, $onload = "", $forceUpdate = false) { //$data also includes any errors
  ...
  $guessAtPk = "";
    ...
    //if($type == "read_only") {
      //var_dump($datum);
      //echo "<HR>";
      $extra = gvfa("extra", $datum);
      //var_dump($extra);
      if(gvfa("autogenerated", $datum) && strpos($extra, "auto_increment") !== false || !gvfa("autogenerated", $datum)) {
        $type = "read_only";
      }
  ...
  $out .= "<input type='hidden' name='_forceupdate' value='" . $forceUpdate . "'/>\n";
...
function splitSqlCommands($sql) {
  ...
  for ($i = 0; $i < $length; $i++) {
      ...
      // Split on semicolon if not inside a string or comment
      if (!$in_string && !$in_single_line_comment && !$in_multi_line_comment && $char === ';') {
          $trimmed = trim($buffer);
          if ($trimmed !== '') {
              $commands[] = $trimmed;
          }
...
function splitSqlCommandsWithComments($sql){
    // 1) Re-use your proven splitter
    $chunks = splitSqlCommands($sql);
    // 2) Inspect each chunk for a leading "--" comment
    $out = [];

    foreach ($chunks as $chunk) {
        $chunk   = ltrim($chunk);        // ignore leading blanks/newlines
        $comment = '';
        $stmt    = $chunk;
        if (preg_match('/^--[^\r\n]*/', $chunk, $m)) {
...
function removeDelimiters($str, $replacement = "_", $delimiters = "|*!") {
