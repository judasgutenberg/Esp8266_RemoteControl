...
if ($_REQUEST) {
    ...
    if ($encryptedKey) {
        ...
        $partiallyDecryptedKey = simpleDecrypt(
            urldecode($encryptedKey),
            strval(chr(countSetBitsInString($data))),
            strval(chr($checksum))
        );
        $storagePassword = simpleDecrypt(
            $partiallyDecryptedKey,
            substr(strval(time() - ceil($averageLatency / 1000)), 1, 8),
            $salt
        );
    } elseif ($encryptedKey2) {
        $timeString = substr(strval(time()), 1, 7); //third param is LENGTH
        //echo $timeString . "<=known to backend";
        //$timeString = "0123227";
        $storagePassword = simpleDecrypt2(
            $encryptedKey2,
            $data,
            $timeString,
            $encryptionScheme
    ...
    if ($user && !$storagePassword) {
        ...
        if (
            ...
            //keeps certain kinds of hacks from working
            die(
                json_encode([
                    "error" =>
                        $user["email"] + ": your brilliant hack has failed",
                ])
            );
        }
    }
    ...
    if ($canAccessData) {
        ...
        $lastInverterRecorded = new DateTime(
            $mostRecentInverterRecord["recorded"]
        );
        //var_dump($mostRecentInverterRecord );
        //echo "<BR>" . $mostRecentInverterRecord  . "***<BR>";
        $ageOfInverterRecord =
            $date->getTimestamp() - $lastInverterRecorded->getTimestamp();
        $useCloudInverterData = false;
        //die($ageOfInverterRecord  . "XX" . $date->getTimestamp() . "YY" . $lastInverterRecorded->getTimestamp());
        if ($ageOfInverterRecord > 600) {
            //it's been five minutes since we last had an inverter record, so maybe use the API then to get it from the SolArk cloud.
        ...
        if (!$conn) {
            ...
            $latestCommandData = getLatestCommandData(
                $locationId,
                $tenant["tenant_id"]
            );
            ...
            if (array_key_exists("data", $_REQUEST)) {
                ...
                if (count($lines) > 1) {
                    ...
                    //first numeric timestamp is ONLY for timestamping old data stored locally
                    if (count($whereAndWhen) > 0) {
                        $numericTimestamp = defaultFailDown(
                            $whereAndWhen[0],
                            "NULL"
                        ); //if a device has been offline, it might decide to timestamp the data it is sending from local storage
                        if (
                            is_numeric($numericTimestamp) &&
                            $numericTimestamp > 0
                        ) {
                            $dt = new DateTime();
                    ...
                    $millis = arrayDefaultFailDown($whereAndWhen, 1, "NULL");
                    $transmissionTimestamp = arrayDefaultFailDown(
                        $whereAndWhen,
                        2,
                        "NULL"
                    );
                    $averageLatency = arrayDefaultFailDown(
                        $whereAndWhen,
                        3,
                        "NULL"
                    );
                    ...
                    $uncertainty = arrayDefaultFailDown(
                        $whereAndWhen,
                        9,
                        "NULL"
                    );
                ...
                if (count($lines) > 2) {
                    $devicePowerInfo = explode("*", $lines[2]);
                    $energyPercentage = arrayDefaultFailDown(
                        $devicePowerInfo,
                        0,
                        "NULL"
                    );
                    $measuredVoltage = arrayDefaultFailDown(
                        $devicePowerInfo,
                        1,
                        "NULL"
                    );
                    $measuredAmpage = arrayDefaultFailDown(
                        $devicePowerInfo,
                        2,
                        "NULL"
                    );
                ...
                if (count($lines) > 6) {
                    ...
                    //var_dump($extraInfo);
                    $mustSaveLastKnownDeviceValueAsValue = arrayDefaultFailDown(
                        $extraInfo,
                        2,
                        false
                    );
                    ...
                    $nonJsonPinData = arrayDefaultFailDown(
                        $extraInfo,
                        4,
                        "NULL"
                    );
                    if (count($extraInfo) > 5) {
                        $justGetDeviceInfo = arrayDefaultFailDown(
                            $extraInfo,
                            5,
                            "NULL"
                        );
                    ...
                    //don't care about changeSourceId either
                }
                if (count($lines) > 7) {
                    $pinValuesKnownToDevice = explode("*", $lines[7]);
                    //var_dump($pinValuesKnownToDevice);
                }
                if (count($lines) > 8) {
                    $recentReboots = explode("*", $lines[8]);
                    foreach ($recentReboots as $rebootOccasion) {
                        if (intval($rebootOccasion) > 0 && $canAccessData) {
                            ...
                            $rebootLogSql =
                                "INSERT INTO reboot_log(device_id, recorded, server_recorded) SELECT " .
                                intval($deviceId) .
                                ",'" .
                                $rebootOccasionSql .
                                "','" .
                                $formattedDateTime .
                                "' 
                  FROM DUAL WHERE NOT EXISTS (SELECT * FROM reboot_log WHERE device_id=" .
                                intval($deviceId) .
                                " AND recorded='" .
                                $rebootOccasionSql .
                                "' LIMIT 1)";
                ...
                if ($mode == "saveIrData") {
                    //data was captured from an irRecorder, so store it in the database!
				...
				VALUES (0, 'new', '" .
                ...
                } elseif ($mode == "commandout") {
                    //if we sent an instant_command, then the ESP8266 will redirect any output it generates to sendRemoteData, sending the output in the "data" parameter
                    ...
                    $sql =
                        "UPDATE command_log SET result_recorded='" .
                        $formattedDateTime .
                        "', result_text='" .
                        mysqli_real_escape_string($conn, $commandText) .
                        "' WHERE tenant_id=" .
                        intval($tenantId) .
                        "  AND device_id=" .
                        intval($deviceId) .
                ...
                } elseif ($mode == "saveLocallyGatheredSolarData") {
                    //used by the special inverter monitoring MCU to send fine-grain data promptly
                    if ($canAccessData) {
                    ...
                    if (count($arrWeatherData) > 4) {
                        //if we actually want to populate the sensor column in device we need to get sensorId now, though now devices can have multiple sensors
            ...
            if ($mode == "kill") {
                $method = "kill";
            } elseif (beginsWith($mode, "getDevices")) {
                ...
                $sql =
                    "SELECT ip_address, name, device_id FROM device  WHERE device_id IN (" .
                    implode(",", $deviceIdsToUse) .
                    ") ORDER BY NAME ASC";
                //echo $sql;
                $result = mysqli_query($conn, $sql);
                if ($result) {
                    ...
                    foreach ($rows as &$row) {
                        ...
                        $sql =
                            "SELECT * FROM device_column_map WHERE device_id=" .
                            $deviceId .
                            " " .
                            $additionalWhere .
                            "  ORDER BY sort_order, display_name, column_name ";
                        $subResult = mysqli_query($conn, $sql);
                        if ($subResult) {
                            $subRows = mysqli_fetch_all(
                                $subResult,
                                MYSQLI_ASSOC
                            );
            ...
            } elseif ($mode == "getEnergyInfo") {
                //this gets critical SolArk data for possible use automating certain things
                ...
                if ($energyInfo) {
                    if (gvfa("errors", $energyInfo)) {
                        ...
                        $out["energy_info"]["pv_power"] =
                            $energyInfo["solar_power"];
                        $out["energy_info"]["bat_power"] =
                            $energyInfo["battery_power"];
                        $out["energy_info"]["gen_power"] =
                            $energyInfo["grid_power"];
                        $out["energy_info"]["load_power"] =
                            $energyInfo["load_power"];
                        $out["energy_info"]["bat_percent"] =
                            $energyInfo["battery_percentage"];
                    }
                }
            } elseif ($mode == "getMap") {
                //an endpoint specifically for maps
                $sql =
                    "SELECT * from device_log WHERE device_id=" .
                    intval($deviceId) .
                    "  ";
                $sql .= buildRestOfSegmentedDataSql(
                    $formattedDateTime,
                    "device_log_id"
                );
                ...
                $deviceSql =
                    "SELECT name, location_name FROM device WHERE device_id = " .
                    intval($deviceId);
            ...
            } elseif ($mode == "getOfficialWeatherData") {
                $sql =
                    "SELECT latitude, longitude  FROM device  WHERE device_id =" .
                    intval($deviceId);
                ...
                if ($latitudeForApi && $longitudeForApi && $apiKey) {
                    $out["official_weather"] = getWeatherDataByCoordinates(
                        $latitudeForApi,
                        $longitudeForApi,
                        $apiKey
                    );
                }
            } elseif ($mode === "getEarliestRecorded") {
                $tableName = filterStringForSqlEntities(gvfw("table"), true);
                $sql =
                    "SELECT MIN(recorded) AS recorded FROM " .
                    $tableName .
                    " WHERE 1=1 ";
                ...
                if (!$conn) {
                    ...
                    if ($sql) {
                        ...
                        if ($result && $canAccessData) {
                            $out["device_features"] = allDeviceFeatures(
                                $tenantId
                            );
                            $out["weather_conditions"] = allWeatherConditions(
                                $tenantId
                            );
                            $out["inverter_data"] = mysqli_fetch_all(
                                $result,
                                MYSQLI_ASSOC
                            );
                            $out["sql"] = $sql;
                ...
                if (!$conn) {
                    ...
                    if ($specificColumn) {
                        ...
                        $sql =
                            "SELECT " .
                            filterStringForSqlEntities($specificColumn, true) .
                            ", device_id, DATE_ADD(recorded, INTERVAL " .
                            $yearsAgo .
                            " YEAR) AS recorded FROM device_log WHERE  device_id IN (" .
                            filterCommasAndDigits($locationIds) .
                            ") ";
                    } else {
                        $weatherColumns = getGraphColumns(
                            $user["tenant_id"],
                            $locationId
                        );
                        $sql =
                            "SELECT " .
                            implode(",", $weatherColumns) .
                            ", device_id, DATE_ADD(recorded, INTERVAL " .
                            $yearsAgo .
                            " YEAR) AS recorded FROM device_log WHERE device_id=" .
                            $locationId;
                    }

                    $sql .=
                        " " .
                        buildRestOfSegmentedDataSql(
                            $formattedDateTime,
                            "device_log_id"
                        );
            ...
            if ($mode == "saveData" || count($multipleSensorArray) > 0) {
                //save data
                ...
                foreach ($multipleSensorArray as $sensorDataString) {
                    //if there is a ! in the weatherInfoString,
                    $arrWeatherData = explode("*", $sensorDataString);
                    if (count($arrWeatherData) > 1) {
                        //it's possible the $weatherInfoString began with a !, meaning the first weather record is technically empty
                        $temperature = mergeWeatherDatum(
                            $consolidateAllSensorsToOneRecord,
                            $temperature,
                            $arrWeatherData,
                            "temperature",
                            $deviceId,
                        ...
                        $windDirection = mergeWeatherDatum(
                            $consolidateAllSensorsToOneRecord,
                            $windDirection,
                            $arrWeatherData,
                            "wind_direction",
                        ...
                        $windIncrement = mergeWeatherDatum(
                            $consolidateAllSensorsToOneRecord,
                            $windIncrement,
                            $arrWeatherData,
                            "wind_increment",
                        ...
                        $precipitation = mergeWeatherDatum(
                            $consolidateAllSensorsToOneRecord,
                            $precipitation,
                            $arrWeatherData,
                            "precipitation",
                        ...
                        $radiationCount = mergeWeatherDatum(
                            $consolidateAllSensorsToOneRecord,
                            $radiationCount,
                            $arrWeatherData,
                            "radiation_count",
                        ...
                        $lightningCount = mergeWeatherDatum(
                            $consolidateAllSensorsToOneRecord,
                            $lightningCount,
                            $arrWeatherData,
                            "lightning_count",
                        ...
                        $illumination = mergeWeatherDatum(
                            $consolidateAllSensorsToOneRecord,
                            $illumination,
                            $arrWeatherData,
                            "illumination",
                        ...
                        $consolidateAllSensorsToOneRecord = mergeWeatherDatum(
                            $consolidateAllSensorsToOneRecord,
                            $consolidateAllSensorsToOneRecord,
                            $arrWeatherData,
                            "consolidate"
                        ...
                        $weatherConditionId = getCurrentWeatherConditionId(
                            $tenant
                        );
                        $weatherSql =
                            "INSERT INTO 
                    ...
                    for (
                        ...
                        if (count($arrWeatherData) > $datumCounter) {
                            ...
                            if (
                                strtolower($testValue) != "null" &&
                                $testValue != "" &&
                                strtolower($testValue) != "nan"
                            ) {
                    ...
                    if (!$doNotSaveBecauseNoData) {
                        //if sensors are all null, do not attempt to store!
                        //echo $weatherSql;
                        if (
                            intval($consolidateAllSensorsToOneRecord) != 1 ||
                            $weatherRecordCounter ==
                                count($multipleSensorArray) - 1
                        ) {
                            ...
                            if ($error != "") {
                                ...
                                //echo "bad" . $weatherSql;
                                logSql(
                                    "bad weather data save sql:" . $weatherSql
                                );
                            }
                        }
                    } else {
                ...
                $out = addNodeIfPresent(
                    addNodeIfPresent(
                        ["message" => "done", "method" => $method],
                        "error",
                        $error
                    ),
                    "sql",
            ...
            if ($mode == "getInitialDeviceInfo") {
                //return a double-delimited string of additional sensors, etc. this one begins with a "*" so we can identify it in the ESP8266. it will be the first data requested by the remote control
                $outString = "*" . deDelimitify($deviceName);
                $sensorSql =
                    "SELECT  f.name, pin_number, power_pin, sensor_type, sensor_sub_type, via_i2c_address, device_feature_id 
					FROM device_feature f 
					LEFT JOIN device_type_feature t ON f.device_type_feature_id=t.device_type_feature_id 
					WHERE  sensor_type IS NOT NULL AND device_id=" .
                    intval($deviceId) .
                    " AND f.enabled ORDER BY sensor_type, via_i2c_address, pin_number;";
                ...
                if ($sensorResult) {
                    ...
                    foreach ($rows as $row) {
                        ...
                        $outString .=
                            ...
                            $row["via_i2c_address"] .
                            "*" .
                            $row["device_feature_id"] .
                            "*" .
                            removeDelimiters($row["name"]) .
                            "*" .
                            $row["overwrite_ordinal"] .
                            "*0";
                    }
                }
                ...
                if ($saveDeviceInfo) {
                    if (strpos($ipAddress, " ") > 0) {
                        //was getting crap from some esp8266s here
                        $ipAddress = explode(" ", $ipAddress)[0];
                    }
                    $deviceSql =
                        "UPDATE device SET ip_address='" .
                        $ipAddress .
                        "', last_poll='" .
                        $formattedDateTime .
                        "' ";
                ...
                $deviceSql =
					...
					FROM device_feature f 
					LEFT JOIN device_type_feature t ON f.device_type_feature_id=t.device_type_feature_id 
					WHERE pin_number IS NOT NULL AND sensor_type IS NULL AND enabled=1 AND device_id=" .
                    intval($deviceId) .
                    " ORDER BY i2c, pin_number;";
                //echo $deviceSql;
                ...
                if ($result) {
                    ...
                    foreach ($rows as $row) {
                        ...
                        $allowAutomaticManagement =
                            $row["allow_automatic_management"];
                        $automationDisabledWhen =
                            $row["automation_disabled_when"];
                        $restoreAutomationAfter =
                            $row["restore_automation_after"];
                        ...
                        $inTheFutureWhenWeRestoreManagement = new DateTime(
                            $automationDisabledWhen
                        );
                        if (intval($restoreAutomationAfter) > 0) {
                            $inTheFutureWhenWeRestoreManagement->modify(
                                $restoreAutomationAfter . " hours"
                            );
                        }
                        $formattedInTheFuture = $inTheFutureWhenWeRestoreManagement->format(
                            "Y-m-d H:i:s"
                        );
                        if (gvfa("debug", $_REQUEST) == "suspension") {
                            echo "<BR>" .
                                ...
                                "; feature id: " .
                                $deviceFeatureId;
                            echo "<BR>formatted in future: " .
                                $formattedInTheFuture;
                            echo "<BR>now time: " . $formattedDateTime;
                            echo "<BR>automationDisabledWhen: " .
                                $automationDisabledWhen;
                            echo "<BR>allowAutomaticManagement: " .
                                $allowAutomaticManagement .
                                "<BR>";
                            echo "<hr>";
                        }
                        if (
                            $allowAutomaticManagement &&
                            ($automationDisabledWhen == null ||
                                $formattedInTheFuture < $formattedDateTime)
                        ) {
                            $automationSql =
                                "SELECT m.* FROM management_rule m JOIN device_feature_management_rule d ON m.management_rule_id = d.management_rule_id AND m.tenant_id=d.tenant_id WHERE d.device_feature_id = " .
                                $deviceFeatureId .
                                " ORDER BY management_priority DESC";
                            //logSql("management sql:" .  $automationSql);
                            $automationResult = mysqli_query(
                                $conn,
                                $automationSql
                            );
                            if ($automationResult) {
                                $automationRows = mysqli_fetch_all(
                                    $automationResult,
                                    MYSQLI_ASSOC
                                );
                                ...
                                foreach ($automationRows as $automationRow) {
                                    $managementRuleIdIfNeeded =
                                        $automationRow["management_rule_id"];
                                    $timeValidStart =
                                        $automationRow["time_valid_start"];
                                    $timeValidEnd =
                                        $automationRow["time_valid_end"];
                                    $conditions = $automationRow["conditions"];
                                    $originalConditions = $conditions;
                                    $managementResultValue =
                                        $automationRow["result_value"];
                                    $managementRuleName =
                                        $automationRow["name"];
                                    $conditions = trim($conditions); // Remove any leading/trailing whitespace
                                    $conditions = mb_convert_encoding(
                                        $conditions,
                                        "UTF-8",
                                        "auto"
                                    );
                                    if (
                                        $timeValidStart == null ||
                                        $timeValidEnd == null ||
                                        ($currentTime >= $timeValidStart &&
                                            $currentTime <= $timeValidEnd)
                                    ) {
                                        ...
                                        foreach (
                                            ...
                                            $tokenContent = substr(
                                                ...
                                                1,
                                                -1
                                            ); //chatgpt gave me too many chars with the regex
                                            //logSql("management token:" .  $tokenContent);
                                            //echo $tokenContent . "\n";
                                            if (
                                                array_key_exists(
                                                ...
                                                if (count($dotParts) > 1) {
                                                    $managementColumn = filterStringForSqlEntities(
                                                        $dotParts[1]
                                                    );
                                                }
                                                $bracketParts = explode(
                                                    "[",
                                                    $dotParts[0]
                                                );
                                                $managementTableName = filterStringForSqlEntities(
                                                    $bracketParts[0]
                                                );
                                                if (count($bracketParts) > 1) {
                                                    $managementLocationId = str_replace(
                                                        "]",
                                                        "",
                                                        $bracketParts[1]
                                                    );
                                                ...
                                                $managmentValueLookupSql =
                                                    "SELECT " .
                                                    $managementColumn .
                                                    " As value FROM " .
                                                    $managementTableName .
                                                    " WHERE recorded >= '" .
                                                    $formattedDateTime20MinutesAgo .
                                                    "' AND " .
                                                    $managementTableName .
                                                    "_id = (SELECT MAX(" .
                                                    $managementTableName .
                                                    ...
                                                    ") " .
                                                    $extraManagementWhereClause;
                                                $managementValueResult = mysqli_query(
                                                    $conn,
                                                    $managmentValueLookupSql
                                                );

                                                if ($managementValueResult) {
                                                    $valueArray = mysqli_fetch_array(
                                                        $managementValueResult
                                                    );
                                                    if ($valueArray) {
                                                        $lookedUpValue = gvfa(
                                                            "value",
                                        ...
                                        if (count($tokenContents) > 0) {
                                            ...
                                            if (
                                                ...
                                                logSql(
                                                    "FAILED TOKEN REPLACEMENT:" .
                                                        $originalConditions
                                                );
                                            }
                                            if (
                                                $managementJudgment == 1 &&
                                                $row["value"] !=
                                                    $managementResultValue
                                            ) {
                        ...
                        if (
                            count($pinValuesKnownToDevice) > $pinCursor &&
                            is_numeric($pinValuesKnownToDevice[$pinCursor])
                        ) {
                            //echo $deviceFeatureId   . "=" . $pinCursor . "<BR>";
                            $lastModified =
                            ...
                            $lastKnownDevice =
                                " last_known_device_value =  " .
                                nullifyOrNumber(
                                    $pinValuesKnownToDevice[$pinCursor]
                                ) .
                                ","; //only do this when we actually have data from the microcontroller
                            $sqlToUpdateDeviceFeature =
                                "UPDATE device_feature SET <lastknowndevice/><lastmodified/><additional/>";
                            $sqlIfDataGoingUpstream =
                                " value =" .
                                $pinValuesKnownToDevice[$pinCursor] .
                                ",";
                            ...
                            //} else {
                            $sqlToUpdateDeviceFeature = str_replace(
                                "<lastknowndevice/>",
                                $lastKnownDevice,
                                $sqlToUpdateDeviceFeature
                            );
                        ...
                        if (
                            $mustSaveLastKnownDeviceValueAsValue ||
                            $automatedChangeMade
                        ) {
                            //actually update the pin values here too!
                        ...
                        if (count($pinValuesKnownToDevice) > $pinCursor) {
                            //do all the logging and the lastModified part
                            if (
                                $row["value"] !=
                                    $pinValuesKnownToDevice[$pinCursor] ||
                                $row["last_known_device_value"] != $row["value"]
                            ) {
                                //we're changing a value by sending one upstream. otherwise we shouldn't change last_known_device_modified
                                $sqlToUpdateDeviceFeature = str_replace(
                                    "<lastmodified/>",
                                    $lastModified,
                                    $sqlToUpdateDeviceFeature
                                ...
                                if (
                                    ...
                                    $allowAutomaticManagement &&
                                    !$automatedChangeMade &&
                                    intval($oldValue) != intval($newValue)
                                ) {
                                    $sqlToUpdateDeviceFeature .=
                                        " automation_disabled_when='" .
                                        $formattedDateTime .
                                ...
                                if (
                                    $historicMechanism == "automation" &&
                                    intval($historicBecame) !=
                                        intval($newValue) &&
                                    $mechanism == $ipAddress &&
                                    $historicRecorded >
                                        $formattedDateTimeAFewMinutesAgo
                                ) {
                                ...
                                $weJustHadALogItemLikeThis =
                                    intval($historicWas) == intval($oldValue) &&
                                    intval($historicBecame) ==
                                        intval($newValue) &&
                                    $historicMechanism == $mechanism &&
                                    $historicRecorded >
                                        $formattedDateTimeAFewMinutesAgo;

                                if (
                                    !$weJustHadALogItemLikeThis &&
                                    $canUpdateDeviceFeature
                                ) {
                                    $loggingSql =
                                        "INSERT INTO device_feature_log (device_feature_id, tenant_id, recorded, beginning_state, end_state, management_rule_id, mechanism, user_id) VALUES (";
                                    $loggingSql .=
                                        nullifyOrNumber(
                                            $row["device_feature_id"]
                                        ) .
                                        ...
                                        intval($newValue) .
                                        "," .
                                        nullifyOrNumber($managementRuleId) .
                                        ",'" .
                                        $mechanism .
                                        "'," .
                                        $userId .
                                    ...
                                    if (
                                        $automatedChangeMade ||
                                        ($specificPin > -1 &&
                                            $specificPin == $pinCursor) ||
                                        $specificPin == -1
                                    ) {
                                        //otherwise we get too much logging if we're in one-pin-at-a-mode time
                                        if (
                                            intval($oldValue) !=
                                            intval($newValue)
                                        ) {
                                            //let's only log ch-ch-ch-changes
                                            $loggingResult = mysqli_query(
                                                $conn,
                                                $loggingSql
                                            );
                                        }
                        ...
                        if (
                            ...
                            $sqlToUpdateDeviceFeature = trim(
                                $sqlToUpdateDeviceFeature
                            );
                            //echo "{" . $sqlToUpdateDeviceFeature . "}" . substr($sqlToUpdateDeviceFeature,-1) . "<BR>";
                            $sqlToUpdateDeviceFeature = removeTrailingChar(
                                $sqlToUpdateDeviceFeature,
                                ",",
                                1
                            );
                            ...
                            if (
                                ...
                                if (
                                    gvfa("debug", $_REQUEST) == "updatefeature"
                                ) {
                                    echo "<BR>automated change: " .
                                        $automatedChangeMade;
                                    echo "<BR>" .
                                        $sqlToUpdateDeviceFeature .
                                        "<BR>";
    ...
    if (endsWith(strtolower($mode), "nonjson")) {
    ...
    } elseif ($nonJsonPinData == "1" && !array_key_exists("error", $out)) {
        //create a very bare-bones non-JSON delimited data object to speed up data propagation to device
        $nonJsonOutString = "|";
                ...
                if ($deviceDatum["enabled"]) {
                    if ($deviceDatum["i2c"] > 0) {
                        $pinName =
                            $deviceDatum["i2c"] .
                            "." .
                            $deviceDatum["pin_number"];
                    } else {
                        $pinName = $deviceDatum["pin_number"];
                    }
                    $nonJsonOutString .=
                        removeDelimiters($deviceDatum["name"]) .
                        "*" .
                        $pinName .
                        "*" .
                        intval($deviceDatum["value"]) .
                        "*" .
                        intval($deviceDatum["can_be_analog"]) .
                        "*" .
                        $deviceDatum["ss"] .
                        "|";
                }
        ...
        if ($latestCommandData) {
            $nonJsonOutString .=
                "!" .
                $latestCommandData["command_id"] .
                "|" .
                removeDelimiters($latestCommandData["command"]) .
                "|" .
                removeDelimiters($latestCommandData["value"]) .
                "|" .
                $latency .
                "!" .
                $latestCommandData["command_log_id"];
        } else {
        ...
        if (
            $justGetDeviceInfo == "1" &&
            array_key_exists("device_data", $out)
        ) {
            //used to greatly limit sent back JSON data to a just-started ESP8266
...
{
    //i wanted this to work so the microcontroller wouldn't have to decompress data but it didn't work
...
function deriveDeviceIdsFromStoragePassword($storagePassword)
{
    ...
    $deviceIds = [];
    $sql =
        "SELECT device_id FROM tenant t JOIN device d ON t.tenant_id=d.tenant_id WHERE storage_password='" .
        mysqli_real_escape_string($conn, $storagePassword) .
...
function deriveTenantFromStoragePassword($storagePassword)
{
    global $conn;
    $sql =
        "SELECT * FROM tenant WHERE storage_password='" .
        mysqli_real_escape_string($conn, $storagePassword) .
        "'";
    $result = mysqli_query($conn, $sql);
    if ($result) {
...
function mergeWeatherDatum(
    ...
    $columnList =
        "temperature,pressure,humidity,gas_metric,wind_direction,wind_speed,wind_increment,precipitation,radiation_count,lightning_count,illumination,color_metric,reserved1,reserved2,reserved3,reserved4,sensor_id,device_feature_id,sensor_name,consolidate";
    ...
    if (count($sourceArray) > $itemNumber) {
        ...
        if ($deviceId && $tenantId) {
            ...
            $lookupKey =
                ...
                "-" .
                $keyName;
            $storageFunction = readMemoryCache(
                $lookupKey,
                $persistTimeInMinutes = 10
            );
            if ($storageFunction === null) {
                $sql =
                    "SELECT storage_function FROM device_column_map WHERE device_id = " .
                    intval($deviceId) .
                    " AND tenant_id=" .
                    intval($tenantId) .
                    " AND table_name='device_log' AND column_name='" .
                    $keyName .
                    "'";
                $result = mysqli_query($conn, $sql);
                if ($result) {
            ...
            if ($storageFunction) {
                ...
                try {
                    ...
                    //this shit does not work. does try/catch ever work in PHP?
                    logSql(
                        "problem with storage function: " . $storageFunction
                    );
                }
            }
        }
    ...
    if (intval($consolidateAllSensorsToOneRecord) == 1) {
        if (
            strtolower($existingValue) == "null" ||
            $existingValue == "" ||
            strtolower($existingValue) == "nan"
        ) {
            if (count($sourceArray) > $itemNumber) {
                $out = $value;
            } else {
...
function deDelimitify($inString)
{
    return str_replace(
        "!",
        "",
        str_replace("|", "", str_replace("*", "", $inString))
    );
}
    ...
    if ($result) {
        ...
        if ($row) {
            ...
            return [
                ...
                "value" => $row["command_data"],
                "command_log_id" => $row["command_log_id"],
            ];
        }
    }
    $sql =
        "SELECT * FROM command c JOIN command_type t ON c.command_type_id=t.command_type_id AND c.tenant_id=t.tenant_id WHERE device_id=" .
        intval($deviceId) .
        " AND c.tenant_id=" .
        $tenantId .
        " AND done=0 ORDER BY command_id ASC LIMIT 0,1";
    $result = mysqli_query($conn, $sql);
    if ($result) {
        $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
        if ($row) {
...
function markCommandDone($commandId, $tenantId)
{
    ...
    $formattedDateTime = $date->format("Y-m-d H:i:s");
    $sql =
        "UPDATE command SET done = 1, performed = '" .
        $formattedDateTime .
        "' WHERE done=0 AND command_id=" .
        intval($commandId) .
        " AND tenant_id=" .
        $tenantId;
    $result = mysqli_query($conn, $sql);
...
function simpleDecrypt($ciphertext, $key, $salt)
{
    ...
    for ($i = 0; $i < $encryptedLength; $i++) {
        ...
        $original =
            $mix ^ ord($key[$i % $keyLength]) ^ ord($salt[$i % $saltLength]);
...
function simpleDecrypt2(
	...
	*/
    ...
    for ($i = 0; $i < strlen($encryptedString); $i += 2) {
        $thisByteOfDataString = ord(
            $dataString[$counter % strlen($dataString)]
        );
        //echo substr($encryptedString, $i, 2) . ":" . $thisByteOfDataString . "\n";
        //echo $i  .  "==\n";

        $thisByteOfStoragePassword = hexdec(substr($encryptedString, $i, 2));
        $thisByteOfTimestamp = ord(
            $timestampString[$counter % strlen($timestampString)]
        );
        ...
        $thisByteResult = generateDecryptedByte(
            $counter,
            $thisNibble,
            $thisByteOfStoragePassword,
            $thisByteOfDataString,
            $thisByteOfTimestamp,
            $dataStringChecksum,
            $timestampStringChecksum,
            $dataStringSetBits,
            $timestampStringSetBits,
            $dataStringZeroCount,
            $timestampStringZeroCount
        );
...
function generateDecryptedByte(
    $counter,
    $thisNibble,
    $thisByteOfStoragePassword,
    $thisByteOfDataString,
    ...
    switch ($thisNibble) {
        ...
        case 3:
            $thisByteResult =
                $thisByteOfStoragePassword ^ $thisByteOfDataString;
            break;
        case 4:
            $thisByteResult =
                $thisByteOfStoragePassword ^
                rotateLeft($thisByteOfDataString, 1);
            break;
        case 5:
            $thisByteResult =
                $thisByteOfStoragePassword ^
                rotateRight($thisByteOfDataString, 1);
            break;
        case 6:
        ...
        case 7:
            $thisByteResult =
                $thisByteOfStoragePassword ^ $timestampStringChecksum;
            break;
        case 8:
        ...
        case 9:
            $thisByteResult =
                $thisByteOfStoragePassword ^ $timestampStringSetBits;
            break;
        case 10:
        ...
        case 15:
            $thisByteResult =
                $thisByteOfStoragePassword ^ $timestampStringZeroCount;
            break;
        default:
...
function buildRestOfSegmentedDataSql($formattedDateTime, $orderBy)
{
    ...
    if ($absoluteTimespanCusps == 1) {
        ...
        $startOfPeriod = sqlForStartOfPeriodScale(
            $periodScale,
            $formattedDateTime
        );
    }
    if ($absoluteTimeAgo) {
        //the default behavior had been to step back into time relatively.  but we can also do it absolutely!
        $sql .=
            "   AND recorded > DATE_ADD('" .
            mysqli_real_escape_string($conn, $absoluteTimeAgo) .
            "', INTERVAL " .
            -1 * intval($yearsAgo) .
            " YEAR)";
    } else {
        $sql .=
            "   AND recorded > DATE_ADD(DATE_ADD(" .
            $startOfPeriod .
            ", INTERVAL " .
            -1 * intval($periodSize * ($periodAgo + 1) + $initialOffset) .
            " " .
            $periodScale .
            " )" .
            ", INTERVAL -" .
    ...
    if ($periodAgo > 0 || $absoluteTimeAgo) {
        $interval = -1 * intval($periodSize * $periodAgo + $initialOffset);
        if ($absoluteTimeAgo) {
            //the default behavior had been to step back into time relatively.  but we can also do it absolutely!
            $interval = 1 * $periodSize;
            $sql .=
                " AND recorded < DATE_ADD(DATE_ADD('" .
                mysqli_real_escape_string($conn, $absoluteTimeAgo) .
                "', INTERVAL " .
            ...
            $sql .=
                ...
                $startOfPeriod .
                ", INTERVAL " .
                -1 * intval($periodSize * $periodAgo + $initialOffset) .
                " " .
                $periodScale .
                " )" .
                ", INTERVAL -" .
    ...
    if ($maxRecorded) {
        $sql .=
            " AND recorded > '" .
            mysqli_real_escape_string($conn, $maxRecorded) .
            "' ";
    }
