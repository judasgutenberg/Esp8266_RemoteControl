    ...
    if($ageOfInverterRecord > 600) { //it's been five minutes since we last had an inverter record, so maybe use the API then to get data from the SolArk cloud. it's not great, but it's enough for automation
      ...
      getCurrentSolarDataFromCloud($tenant); //only do this one tenth of the time
		...
		if(!$conn) {
            ...
            $transmissionTimestamp = arrayDefaultFailDown($whereAndWhen, 2, "NULL");
            $averageLatency = arrayDefaultFailDown($whereAndWhen, 3, "NULL");
            ...
            if($averageLatency) {
              writeMemoryCache("latency-" . $deviceId, $averageLatency);
					...
					if(count($lines) > 2) {
						...
						$energyPercentage = arrayDefaultFailDown($devicePowerInfo, 0, "NULL");
            $measuredVoltage = arrayDefaultFailDown($devicePowerInfo, 1, "NULL");
            $measuredAmpage = arrayDefaultFailDown($devicePowerInfo, 2, "NULL");
					}
            ...
            $nonJsonPinData = arrayDefaultFailDown($extraInfo, 4, "NULL");
						if(count($extraInfo)>5) {
							$justGetDeviceInfo = arrayDefaultFailDown($extraInfo, 5, "NULL");
...
function buildRestOfSegmentedDataSql($formattedDateTime, $orderBy) {
  ...
  if ($periodAgo > 0  || $absoluteTimeAgo) {
    ...
    if($absoluteTimeAgo) {//the default behavior had been to step back into time relatively.  but we can also do it absolutely!
      ...
      $sql .= " AND recorded < DATE_ADD(DATE_ADD('" . mysqli_real_escape_string($conn, $absoluteTimeAgo)  . "', INTERVAL " . $interval . " " . $periodScale . " )" . ", INTERVAL -" . intval($yearsAgo) . " YEAR)";
    } else {
